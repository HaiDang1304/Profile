  ...
  return (
    <div className="portfolio-shell">
      ...
      <main>
        ...
        </div><div className="hero-bottom section-grid"><div className="scroll-hint"><span>{t.hero.scroll}</span><i /></div><div className="hero-stats">{t.hero.stats.map(([value, label]) => <div key={label}><b>{value}</b><span>{label}</span></div>)}<div key="views"><b>{pageViews.toString().padStart(2, '0')}{pageViews > 0 ? '+' : ''}</b><span>{lang === 'vi' ? 'LƯỢT TRUY CẬP' : 'VISITORS'}</span></div></div></div></section>
        ...
        <section id="about" className="content-section about-section"><div className="section-grid"><div className="section-heading reveal"><span className="section-tag">{t.about.tag}</span><h2>{t.about.title}</h2><p>{t.about.lead}</p></div><div className="about-layout"><div className="about-story pixel-window reveal"><div className="window-bar"><span>ABOUT_ME.TXT</span><div><i /><i /><i /></div></div><p>{profile?.[lang === 'vi' ? 'bio_vi' : 'bio_en'] || t.about.body}</p><div className="signature" style={{position:"relative"}}>{displayName}<span>_</span></div><a className="pixel-button" href={`mailto:${profile?.email || 'haidanglu2004@gmail.com'}?subject=CV%20request`}>{t.about.download}<ArrowDownRight size={17} /></a></div><div className="journey-card reveal"><Sparkles size={22} /><span>{t.about.journey}</span><p>{t.about.journeyText}</p><div className="journey-track"><i /><i /><i /><i /></div></div></div><div className="value-grid">{t.about.values.map(([number, title, text]) => <article className="value-card reveal" key={number}><span>{number}</span><Zap size={20} /><h3>{title}</h3><p>{text}</p></article>)}</div></div></section>
